...
class ReplayBuffer(object):
	def __init__(self, obs_space, action_space, max_size=int(1e5)):
		...
		self.time = np.zeros((max_size, *obs_space[4].shape))
		...
		self.next_state_time = np.zeros((max_size, *obs_space[4].shape))
	...
	def add(self, state, action, next_state, reward, done):
		...
		self.next_state_time[self.ptr] = next_state[4]
	...
	def sample(self, batch_size):
		...
		return (
			...
			torch.FloatTensor(self.time[ind]).to(self.device),
            ...
            torch.FloatTensor(self.next_state_time[ind]).to(self.device),
